...
)
{
    ...
    for (int i = 0; i < n; i++)
    {
        data_t g_q =
            (Fc[i] + (Fs[i] - Fc[i]) *
            std::exp(-std::pow(std::abs(dq[i] / vs[i]), delta_s[i])))
            * std::tanh(dq[i]);

        data_t vis_q = Fv[i] * dq[i];

        tau_f_out[i + 1] = g_q + vis_q;
    }
}
...
void total_tau(
    ...
    const data_t sinq[n],
    const data_t cosq[n],
    data_t tau_total[n+2]
)
{
    data_t tau_dyn[n+2]  = {0};
    data_t tau_fric[n+2] = {0};
    ...
    for (int i = 1; i <= n; i++)
    {
        tau_total[i] = tau_dyn[i] + tau_fric[i];
    }
}